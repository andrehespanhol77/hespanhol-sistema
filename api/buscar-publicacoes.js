...
async function sendAlertEmail(p) {
  await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: 'sistema@andrehespanhol.com', to: 'andrehespanhol@andrehespanhol.com',
...
module.exports = async (req, res) => {
  ...
  for (const oab of OABS) {
    ...
    for (const item of items) {
      ...
      const { error: ie } = await supabase.from('publicacoes').insert({
      ...
      if (ie) { stats.erros.push(ie.message); continue; }
      stats.inseridas++;
